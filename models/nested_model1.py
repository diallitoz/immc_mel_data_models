...
MarginalCostPT_scaled = DefineVariable('MarginalCostPT_scaled',MarginalCostPT / 10 , database)
CostCarCHF_scaled = DefineVariable('CostCarCHF_scaled', CostCarCHF / 10 , database)
distance_km_scaled = DefineVariable('distance_km_scaled', distance_km / 5 , database)
...
# 1: nests parameter
MU_NOCAR = Beta('MU_NOCAR', 1.0, 1.0, None, 0) ###Don't Keep this parameter fixed to the initial value
...
logprob = models.lognested(V, None, nests, Choice)###Non prise en compte de la disponibilité des modes (av)
...
biogeme.modelName = '../output/01nestedEstimation'
